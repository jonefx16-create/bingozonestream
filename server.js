...
app.post('/api/admin/finance-report', auth, async (req, res) => {
    try {
        ...
        txs.forEach(tx => { if(tx.type === 'deposit') totalDeposit += tx.amount; if(tx.type === 'withdraw') totalWithdraw += tx.amount; });
...
function startCountdown() {
    gameState = "WAITING"; activePlayers = {}; totalPrizePool = 0; totalTickets = 0; calledNumbers = []; currentDrawSequence = [];
    ...
    gameInterval = setInterval(() => {
        ...
        io.emit('game_status', { state: gameState, timer, totalPrizePool, totalTickets, ticketPrice: GLOBAL_SETTINGS.ticketPrice, calledNumbers, playersCount: Object.keys(activePlayers).length, gameId, globalTakenTickets });
        if (timer <= 0) { clearInterval(gameInterval); startGame(); }
    }, 1000);
}

function startGame() {
    // 🟢 ቢያንስ 2 ተጫዋች ከሌለ ጌሙ አይጀምርም
    if (Object.keys(activePlayers).length < 2) { startCountdown(); return; }
    
    gameState = "PLAYING"; currentDrawSequence = generateRiggedDrawSequence(); 
    io.emit('game_status', { state: gameState, timer: "LIVE", totalPrizePool, totalTickets, calledNumbers, playersCount: Object.keys(activePlayers).length, gameId, globalTakenTickets });
    gameInterval = setInterval(() => {
        if (gameState !== "PLAYING" || currentDrawSequence.length === 0) { clearInterval(gameInterval); setTimeout(startCountdown, 5000); return; }
...
io.on('connection', (socket) => {
    socket.emit('game_status', { state: GLOBAL_SETTINGS.isGamePaused ? "PAUSED" : gameState, timer: gameState === "PLAYING" ? "LIVE" : timer, totalPrizePool, totalTickets, ticketPrice: GLOBAL_SETTINGS.ticketPrice, calledNumbers, playersCount: Object.keys(activePlayers).length, gameId, globalTakenTickets });
