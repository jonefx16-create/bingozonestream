...
io.on('connection', (socket) => {
    socket.emit('game_status', { state: gameState, timer: gameState === "PLAYING" ? "LIVE" : timer, totalPrizePool, totalTickets, calledNumbers, gameId, ticketPrice: GLOBAL_SETTINGS.ticketPrice });
...
app.post('/api/admin/live-stats', auth, async (req, res) => {
    res.json({ totalPlayers: Object.keys(activePlayers).length, totalTickets, totalPrizePool, gameState, gameId });
});
